...
<syntax.Exp>           := <AddExp>
<Cond>          := <LOrExp>
<LVal>          := Ident { '[' <syntax.Exp> ']' } // 普通变量，一维或二维数组
<PrimaryExp>    := '(' <syntax.Exp> ')' | <LVal> | <Number> // 三种情况, 子表达式, 左值, 字面量
<Number>        := IntConst
<UnaryExp>      := <PrimaryExp> | <Ident> '(' [ <FuncRParams> ] ')' | <UnaryOp> <UnaryExp> // PrimaryExp 或者 FunctionCall 或者含有一元运算符
<UnaryOp>       := '+' | '-' | '!'  // '!' 仅能在条件表达式中出现
<FuncRParams>   := <syntax.Exp> { ',' <syntax.Exp> } 
...
```
<LVal>          := Ident { '[' <syntax.Exp> ']' } // public class LVal { ident, class Index, List<Index> }
...
<FuncRParams>   := <syntax.Exp> { ',', <syntax.Exp> } // List<syntax.Exp>
...
<syntax.Exp>           := <AddExp> // public class syntax.Exp extends AddExp { }
...
处理完以上成分之后还有两个 `<syntax.Exp>` 和 `<Cond>`，这两种节点只是在 `<AddExp>` 和 `<LOrExp>` 上套了一层，为了输出方便还是创建相应的类，持有其套的类的实例并重写输出方法。对于 `ConstExp` ，将其作为 `syntax.Exp` 的子类。
...
<Stmt> := <LVal> '=' <syntax.Exp> ';'
    ...
    | 'return' [<syntax.Exp>] ';'
    | <LVal> '=' 'getint' '(' ')' ';'
    | 'printf' '(' FormatString { ',' <syntax.Exp> } ')' ';'
...
```
// 以分号结尾的简单语句
<AssignStmt>    := <LVal> '=' <syntax.Exp> // 这些新增非终结符的组成不含分号
<ExpStmt>       := <syntax.Exp>            // 空语句放到最后了
<BreakStmt>     := 'break'
<ContinueStmt>  := 'continue'
<ReturnStmt>    := 'return' [<syntax.Exp>]
<InputStmt>     := <LVal> '=' 'getint' '(' ')'
<OutputStmt>    := 'printf' '(' FormatString { ',' <syntax.Exp> } ')'
...
在改写后的 `Stmt` 相关文法中，大多数分支只需要向前看 1 个符号即可够用，但判断 `AssignStmt`, `InputStmt` 和 `ExpStmt` 需要面临 `LVal` 与 `syntax.Exp` 的选择，在这里向前看的符号数量是难以确定的。回顾前一节刚刚写完的表达式部分，可以看出 `LVal` 是 `syntax.Exp` 的"子集"（即，若一个短语能够由 `LVal` 推导出来，则一定能被 `syntax.Exp` 推导），因此在向前看完其他符号后，直接进行一个 `syntax.Exp` 的 parse，由于递归下降对语法成分的匹配是贪心的（尽可能多匹配），因此若解析出来的 `syntax.Exp` 仅含有一个 `LVal` 则一定是 `LVal`，反之则不是，由此即解决了 `LVal` 和 `syntax.Exp` 的区分问题，进而完成语句部分的语法树节点类建模和解析器的编写。
...
<InitVal>       := <syntax.Exp> | '{' [ <InitVal> { ',' <InitVal> } ] '}'
```
...
对于需要向前看符号才能确定的语法成分，令其解析方法以参数的形式接受其父成分解析程序中向前看取出的符号，而无需再用迭代器取出一遍。（通常的语法成分解析方法入口是不需要传参的，只需要访问读取头迭代器就可以了。） 但对于 `syntax.Exp` 等出现位置较为复杂的语法成分，其可能出现在多种不同的父成分下，不同的父成分解析 `syntax.Exp` 需要向前看的符号数量也不同，如果仍然采用先取出符号再传参数的方式，`syntax.Exp` 及其子成分的解析程序将会较为混乱，为了保持架构的完整性，这里就需要用上 `ListIterator` 的双向特性，若向前看取出的符号不满足其他分支，而接下来又需要走向一个 `syntax.Exp` 分支，则回退读取头迭代器，将向前看的符号 "放回去"，再调用 `parseExp` 进行解析。（这种回退并不等同于回溯，可以理解为受限于 Java 迭代器功能的一种折中选择）